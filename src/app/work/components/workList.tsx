...
const WorkList = ({ projects }: WorkListProps) => {
  ...
  useEffect(() => {
    ...
    console.log(
      '[Component] WorkList: Available projects:',
      projects.map((p) => ({
        id: p.id,
        title: p.title,
      })),
    ...
    <div className="mb-32 grid text-center p-6 lg:max-w-7xl gap-2 lg:mb-0 lg:grid-cols-2 lg:text-right lg:self-end">
      {projects.map((project) => {
        ...
        return (
          <div
            key={project.id}
            className={`transition-opacity duration-1500 ease-out-cubic ${isCurrentProject ? 'opacity-100' : 'opacity-40'}`}
          >
            <Link href={href}>
              <h2
                className={`group p-2 bg-black text-white px-5 relative transition-colors hover:bg-white hover:text-black align-middle m-2 text-4xl font-semibold`}
              >
                {project.title}
              </h2>
            </Link>
